...
VERSION_NAME := $(shell sed -n 's/^VERSION_NAME=//p' gradle.properties | tail -n 1)
RELEASE_APK := $(RELEASE_DIR)/NetworkStreamViewer-v$(VERSION_NAME).apk
LEGACY_RELEASE_APK := $(RELEASE_DIR)/app-release.apk
...
release:
	@rm -f '$(RELEASE_APK)' '$(LEGACY_RELEASE_APK)'
